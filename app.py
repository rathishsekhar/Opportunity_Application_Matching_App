...
# Setting folder path - for local use only
...
# Function to access a file in nested folders within a GCS bucket
def get_data_gcs(file_name, bucket_name = 'opportunity-application-matcher'):
    client = storage.Client()  # Create a GCS client
    bucket = client.get_bucket(bucket_name)  # Get the bucket object
    file_path = 'streamlit_app_deploy_stack/data/input/' + str(file_name) + '.pkl'  # Full path for project including folders
    blob = bucket.blob(file_path)  # File path including the input folders
    data = blob.download_as_bytes()  # Gather pickle data as bytes
    read_file = pickle.load(io.BytesIO(data))  # Read the gathered pickle file
    return read_file

# Setting model and stacking methods
model = 'w2v'  # Change this to bert or dbert or w2v whenever required
stack = 'vertical' # Change this to horizontal or vertical as required
